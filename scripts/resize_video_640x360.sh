...
The output defaults to:
  data/converted/<input-name>_360p.mp4

Conversion settings:
  - Resolution: 640x360 (16:9 input)
  - Frame rate: 30 FPS
...
if [[ $# -eq 2 ]]; then
    ...
    OUTPUT_VIDEO="${PROJECT_ROOT}/data/converted/${INPUT_STEM}_360p.mp4"
fi
...
echo "Converting to 640x360 at 30 FPS..."

ffmpeg \
    -nostdin \
    -i "${INPUT_VIDEO}" \
    -vf "scale=640:-2:flags=lanczos,fps=30" \
    -c:v libx264 \
    -preset fast \
    -crf 20 \
    -c:a copy \
